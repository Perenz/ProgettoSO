...
void get_info_string(char* ans){//TODO aggiungere timer
    ...
    status_string = status==1? "aperto" : "chiuso";
    sprintf(ans, "fridge %d %d %s %.2f %.2f %d %d \n", pid, id, status_string, tempoSecondi, delay, riempimento, temperatura);//TODO aggiungere timer
    //free(status_string);
