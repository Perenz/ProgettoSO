...
int dev_switch(char **args){
    ...
    printf("%d", id_change);
    if(id_change == id && (strcmp(args[2], "apertura")==0 || strcmp(args[2],"chiusura")==0) &&
        (strcmp(args[3], "on")==0 || strcmp(args[3], "off")==0)){
        set_time();
        if(strcmp(args[2], "apertura")==0){
            if(strcmp(args[3], "on")==0)
                status = 1;
        }else{ //finestra già aperta/si prova a chiuderla
            if(strcmp(args[3], "on")==0){
                status = 0;
            }
        }
...
void get_info_string(char* ans){//TODO aggiungere timer
    ...
    status_string = status==1? "aperta" : "chiusa";
    sprintf(ans, "window %d %d %s %.2f\n", pid, id, status_string, tempoSecondi);//TODO aggiungere timer
    //free(status_string);
...
void set_info(char* info){
    ...
    if(strcmp(info_split[0], "default")==0){
        ...
        if(strcmp(info_split[3], "aperta")==0){
            status = 1;
        }else{//spenta
            status = 0;
        }
