...
int cen_delete(char **args, NodoPtr collegati_list, NodoPtr magazzino_list){
    ...
    if(args[1]==NULL ){
        ...
        signal(SIGCONT, sign_cont_handler);
        cmd comando;
        comando.tipo_comando = 'd'; //imposto il comando
        comando.forzato = 0;

        if(strcmp(args[1], "--all")==0){ //con --all elimino tutto
            comando.forzato = 1;
        }else{
            comando.forzato = 0;
...
int cen_switch(char **args, NodoPtr collegati_list, NodoPtr magazzino_list){
    ...
    if(args[1]==NULL || args[2]==NULL || args[3]==NULL){      
        ...
        cmd comando;
        comando.forzato = 0;
        comando.tipo_comando = 's'; //imposto il comando (switch)
        comando.manuale = 0;
        comando.id = atoi(args[1]); 
        
        strcpy(comando.cmdInterruttore.nome,args[2]); //nome interruttore
        strcpy(comando.cmdInterruttore.stato,args[3]);  //stato interruttore
        //controllo che il nome dell'interruttore sia uno tra qulli disponibili
...
int cen_link(char** args, NodoPtr collegati_list, NodoPtr magazzino_list){
    ...
    id_dst = atoi(args[3]);
    int i;
    //LINKING CON DESTINAZIONE LA CENTRALINA 
