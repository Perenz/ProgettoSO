...
risp manualCen_info(char *arg, NodoPtr collegati_list, NodoPtr magazzino_list){
    risp* array_risposte_collegati_list;
    risp* array_risposte_magazzino_list;
    //Liste per l'ascolto della risposta
    malloc_array(&array_risposte_collegati_list, 1);
    malloc_array(&array_risposte_magazzino_list, 1);

    //setto signal handler per SIGCONT
    signal(SIGCONT, sign_cont_handler);
    //Creo il comando e gli assegno i valore necessari
    cmd comando;
    risp dispCercato;
...
int cen_power(char **args, NodoPtr collegati_list, NodoPtr magazzino_list){
    ...
    if(powerOn==1){ //è acessa allora la spengo
        ...
        //Segnali per indicare a tutti i dispositivi dello spegnimento della centralina
        nodo=collegati_list;
        while(nodo!=NULL){
        ...
        //Segnale per indicare ai figli la ripresa della centralina
        nodo=collegati_list;
        while(nodo!=NULL){
